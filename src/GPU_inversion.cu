...
int main()
{
   ...
   // transformation<<<(knots.size()+255)/256,256>>>(knots.size(),p,d_knotsvector);
    ...
    init_randompoint_for_inversion(test_point,cnt_point);
    // init_ground_truth(cnt_point);
    // for (int i = 0; i < cnt_point; i++) {
    //     test_point[i]=evaluate_Bspline(knots,control_points,p,control_points.size(),ground_truth[i]);
    // }
    // test_point[0]=control_points[0];
    // test_point[1]=control_points[control_points.size()-1];
    ...
    test1=evaluate_Bspline(knots,control_points,p,control_points.size(),0.0);
    test2=evaluate_Bspline(knots,control_points,p,control_points.size(),0.5);
    test3=evaluate_Bspline(knots,control_points,p,control_points.size(),1.0);
    printf("test1=(%f,%f,%f,%f)\n",test1.x,test1.y,test1.z,test1.w);
    printf("test2=(%f,%f,%f,%f)\n",test2.x,test2.y,test2.z,test2.w);
    printf("test3=(%f,%f,%f,%f)\n",test3.x,test3.y,test3.z,test3.w);

    for(int i=0;i<10;i++)
    {
        printf("ground_truth[%d]=%f\t",i,ground_truth[i]);
        printf("test_point[%d] = %f %f %f %f\n",i,test_point[i].x,test_point[i].y,test_point[i].z,test_point[i].w);
    }
